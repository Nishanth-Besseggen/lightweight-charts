...
export class Magnet {
	...
	public align(price: number, index: TimePointIndex, pane: Pane): number {
		...
		const serieses: readonly Series[] = pane.dataSources()
		.filter((ds: IPriceDataSource)=> ds instanceof Series)
		.map((ds: IPriceDataSource)=> ds as unknown as Series)
		...
		const candidates = serieses.reduce(
			(acc: Coordinate[], series: Series) => {
				if (pane.isOverlay((series as unknown) as IPriceDataSource) || !series.visible()) {
					return acc;
				}
