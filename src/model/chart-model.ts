...
export class ChartModel implements IDestroyable {
	...
	public moveSeriesToScale(series: Series, targetScaleId: string): void {
		const pane = ensureNotNull(this.paneForSource((series as unknown) as IPriceDataSource));
		pane.removeDataSource((series as unknown) as IPriceDataSource);
		...
		if (target === null) {
			...
			pane.addDataSource((series as unknown) as IPriceDataSource, targetScaleId, zOrder);
		} else {
			// if move to the new scale of the same pane, keep zorder
			// if move to new pane
			const zOrder = (target.pane === pane) ? series.zorder() : undefined;
			target.pane.addDataSource((series as unknown) as IPriceDataSource, targetScaleId, zOrder);
		}
	}
	...
	public moveSeriesToPane(series: Series, fromPaneIndex: number, newPaneIndex: number): void {
		...
		const previousPane = this.paneForSource((series as unknown) as IPriceDataSource);
		previousPane?.removeDataSource((series as unknown) as IPriceDataSource);
