...
export class TimeAxisWidget implements MouseEventHandlers, IDestroyable {
	...
	public paint(type: InvalidationLevel): void {
		...
		if (topTarget !== null) {
			...
			this._drawLabels([...(this._chart.model().serieses().map((series):IDataSource=>(series as unknown) as IDataSource)), (this._chart.model().crosshairSource() as unknown) as IDataSource], topTarget);
	...
	private _drawAdditionalSources(target: CanvasRenderingTarget2D, axisViewsGetter: ITimeAxisViewsGetter): void {
		...
		for (const source of sources) {
			drawSourcePaneViews(
				...
				source as unknown as IDataSource,
