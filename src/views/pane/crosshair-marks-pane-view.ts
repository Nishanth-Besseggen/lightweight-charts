...
export class CrosshairMarksPaneView implements IUpdatablePaneView {
	...
	private _updateImpl(pane: Pane): PaneRendererMarks[] {
		...
		const serieses = this._chartModel.serieses()
		.map((datasource: Series, index: number) : [Series, number] =>  [datasource, index])
		.filter((entry: [Series, number]) => pane.dataSources().includes((entry[0] as unknown) as IPriceDataSource))
